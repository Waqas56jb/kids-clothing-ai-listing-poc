        ...
        >
          <motion.div
            ...
            className={`flex w-full items-center justify-between gap-3 rounded-[1.75rem] border px-2.5 py-2 shadow-elevated backdrop-blur-2xl transition-colors duration-500 ${
              scrolled ? 'border-white/10 bg-ink/90 text-white' : 'border-white/15 bg-ink/55 text-white'
            }`}
          >
            <a href="#top" className="flex items-center gap-2.5 pl-1" onClick={() => setMobileOpen(false)}>
              <TiltCard maxTilt={12} glare={false} className="shrink-0">
                <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-sand/25 to-white/5 ring-1 ring-white/20">
                  <Baby className="h-5 w-5 text-sand" />
                </span>
              </TiltCard>
            ...
            <div className="hidden items-center gap-2 pr-1 md:flex">
              <Link to="/login" className="rounded-full px-4 py-2 text-[13px] font-semibold text-white/75 transition hover:bg-white/10 hover:text-white">
                Logga in
              </Link>
              <Link
          ...
          >
            ...
            >
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 border-t border-white/10 pt-3">
                <Link to="/login" onClick={() => setMobileOpen(false)} className="rounded-2xl bg-white/10 px-3 py-3 text-center text-sm font-semibold text-white">
                  Logga in
                </Link>
                <Link to="/login?mode=signup" onClick={() => setMobileOpen(false)} className="rounded-2xl bg-sand px-3 py-3 text-center text-sm font-semibold text-ink">
                  Börja sälja
                </Link>
              </div>
            </motion.div>
