...
import { categoryLabel, jobStatusLabel, matchStatusLabel, STAGE_SV } from '../../lib/sv'
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-2xl font-bold text-slate-800 sm:text-3xl">Projekt {job.job_id}</h1>
        <Badge tone={STATUS_TONE[job.status] ?? 'neutral'}>{jobStatusLabel(job.status)}</Badge>
      </div>

      {(job.status === 'processing' || job.status === 'queued') && (
        <Card className="mt-5 p-5">
          <p className="text-sm font-medium text-slate-700">Bearbetas – {STAGE_SV[job.stage] ?? job.stage ?? 'startar'}</p>
          {job.total > 0 && (
      ...
      {job.status === 'error' && (
        <Card className="mt-5 flex items-start gap-2.5 border-rose-100 bg-rose-50 p-5 text-sm text-rose-700">
          ...
          <span>Bearbetningen misslyckades: {job.error}</span>
        </Card>
      )}
      ...
      {job.status === 'done' && (
        <>
          <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard label="Bilder" value={job.image_count} icon={Camera} tone="brand" />
            <StatCard label="Detektioner" value={job.result.total_detections} icon={ScanSearch} tone="brand" />
            <StatCard label="Plagg" value={garments.length} icon={Package} tone="emerald" />
            <StatCard label="Behöver granskas" value={needsReview} icon={AlertTriangle} tone="amber" />
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <Link to={`/garments/${jobId}`} className="flex items-center gap-1.5 rounded-xl bg-white px-4 py-2 text-sm font-semibold text-brand-700 shadow-soft hover:bg-brand-50">
              <ScanSearch className="h-4 w-4" /> Plagghantering
            </Link>
            ...
            <Link to={`/groups/${jobId}`} className="flex items-center gap-1.5 rounded-xl bg-white px-4 py-2 text-sm font-semibold text-brand-700 shadow-soft hover:bg-brand-50">
              <Package className="h-4 w-4" /> Grupper & paket
            </Link>
          ...
          <h2 className="mt-8 font-display text-lg font-bold text-slate-800">Hittade plagg</h2>
                  ...
                  <div className="space-y-1.5 p-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      <Badge tone={toneForMatch(garment.match_status)} className="text-[10px]">
                        {matchStatusLabel(garment.match_status)}
                      </Badge>
